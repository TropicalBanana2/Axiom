...
[zombs.io](https://zombs.io). Run headless bots that stay logged in
after your browser closes, drive them from a dashboard, and script the
in-game panel from a single JSON schema.

The repo holds three components:

| Path | What | Status |
|------|------|--------|
| **`server/`** | The product. A localhost install (3 pm2 processes) that serves a modded play page, runs persistent headless bots, and exposes a dashboard for spawning/attaching/coordinating them. | **Active** |
| `editor/` | **uiengine** — a Vite + React + Monaco app for visually editing the panel schema and authoring the scripts behind each control. Exports a userscript. | Companion |
| `axiom.user.js` | Standalone Tampermonkey/Violentmonkey userscript — the original client-only delivery, generated by the editor. | Legacy |

`server/` is what you run. The `editor/` and `axiom.user.js` predate it
and share the same schema format, so anything authored in uiengine can
be served by the server's `/api/schema`.
...
- **Pathfinding navigation** — a windowed A\* pathfinder keeps each bot
  shuttling only between its base (its spawn position) and its set farm
  spot, with day/night-aware travel so it isn't caught out by zombies.
...
A visual editor for the panel schema.
...
It edits the schema, live-previews the panel, and exports a userscript.
The same schema drives the server's in-game panel.

### Userscript (`axiom.user.js`)

The original client-only delivery. Install in Tampermonkey/Violentmonkey
and press `` ` `` on zombs.io to toggle the panel. Superseded by the
server's `/play` page but kept for client-only use.
...
├── editor/             uiengine — schema editor (Vite + React + Monaco)
└── axiom.user.js       standalone userscript
```
...
the bots dialing zombs.io, the leaderboard proxy, and — if you use
uiengine's optional Claude integration — calls you trigger to
`api.anthropic.com`.
